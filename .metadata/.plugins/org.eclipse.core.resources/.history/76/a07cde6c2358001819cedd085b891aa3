...
public class CreationCommande extends HttpServlet
{
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
	{
		...
		if (nom.trim().isEmpty() || adresse.trim().isEmpty() || telephone.trim().isEmpty() || montant == -1
			...
			message = "Erreur - Vous n'avez pas rempli tous les champs obligatoires. <br> <a href=\"/filRouge/commande\">Cliquez ici</a> pour accéder au formulaire de création d'une commande.";
		else
		...
		getServletContext().getRequestDispatcher("/WEB-INF/creationCommande.jsp").forward(request, response);
	}
}